...
def voice_preview(request):
    ...
    # Cache previews to R2 so they survive deploys (and are CDN-served
    # when reused). Cache key is the same hash; we just check R2 first
    # and fall through to generating + uploading on miss.
    from django.core.files.storage import default_storage
    from .services.r2_storage import upload_to_r2
    import requests as _requests

    cache_key = hashlib.md5(f"{voice}:{text}".encode()).hexdigest()
    r2_path = f"voice_previews/{cache_key}.mp3"

    if default_storage.exists(r2_path):
        try:
            cache_url = default_storage.url(r2_path)
            r = _requests.get(cache_url, timeout=15)
    ...
    # Best-effort upload to R2. If R2 is down, still return the freshly
    # generated bytes — caller doesn't care where the cache lives.
    try:
        upload_to_r2(audio_bytes, r2_path, content_type='audio/mpeg')
    except Exception as e:
        logger = __import__('logging').getLogger(__name__)
        logger.warning(f"Voice preview R2 cache upload failed: {e}")
